...
ARG BASE_IMG=openjdk:11-jre-slim

FROM ${BASE_IMG}

ARG JAR_FILE=./target/remote-shuffle-service-server.jar
ARG spark_uid=185
...
RUN set -ex && \
    mkdir -p /opt/remote-shuffle-service

WORKDIR /opt/remote-shuffle-service/
