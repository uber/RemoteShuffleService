 ...
 */
final class RssShuffleInMemorySorter {

    private static final class SortComparator implements Comparator<RssPackedRecordPointer> {
        @Override
        public int compare(RssPackedRecordPointer left, RssPackedRecordPointer right) {
     ...
     * {@link RssPackedRecordPointer}. The sort operates on this array instead of directly manipulating
     * records.
     *
    ...
    RssShuffleInMemorySorter(MemoryConsumer consumer, int initialSize, boolean useRadixSort) {
    ...
    public void reset() {
        ...
        if (consumer != null) {
            ...
            // data to `RssShuffleInMemorySorter` when `array` is `null` (e.g., in
            // RssShuffleExternalSorter.growPointerArrayIfNecessary, we may try to access
            // `RssShuffleInMemorySorter` when `allocateArray` throws SparkOutOfMemoryError).
            array = null;
            usableCapacity = 0;
     ...
     *                      {@link RssPackedRecordPointer#MAXIMUM_PAGE_SIZE_BYTES} bytes of a data page.
     * @param partitionId the partition id, which must be less than or equal to
     *                    {@link RssPackedRecordPointer#MAXIMUM_PARTITION_ID}.
     */
    public void insertRecord(long recordPointer, int partitionId) {
        if (!hasSpaceForAnotherRecord()) {
            throw new IllegalStateException("There is no space for new record");
        }
        array.set(pos, RssPackedRecordPointer.packPointer(recordPointer, partitionId));
        pos++;
    }
     ...
     */
    public static final class RssShuffleSorterIterator {

        private final LongArray pointerArray;
        private final int limit;
        final RssPackedRecordPointer packedRecordPointer = new RssPackedRecordPointer();
        private int position = 0;

        RssShuffleSorterIterator(int numRecords, LongArray pointerArray, int startingPosition) {
     ...
     */
    public RssShuffleSorterIterator getSortedIterator() {
        int offset = 0;
        if (useRadixSort) {
            offset = RadixSort.sort(
                    array, pos,
                    RssPackedRecordPointer.PARTITION_ID_START_BYTE_INDEX,
                    RssPackedRecordPointer.PARTITION_ID_END_BYTE_INDEX, false, false);
        } else {
            ...
            Sorter<RssPackedRecordPointer, LongArray> sorter =
                    new Sorter<>(new RssShuffleSortDataFormat(buffer));

            sorter.sort(array, 0, pos, SORT_COMPARATOR);
        }
        return new RssShuffleSorterIterator(pos, array, offset);
    }
}
