 ...
 */
private[rss] class WriterAggregationImpl[K, V, C](shuffleWriteCurator: ShuffleWriteCurator,
  ...
  def recordsWritten: Int = recordsWrittenCnt
  ...
  private[rss] def spillMap(): Unit = {
    ...
    val spillDataItr = new Iterator[(Int, Array[Byte], Int)] {
      ...
      override def next(): (Int, Array[Byte], Int) = {
        ...
        // Clear the output buffer for data present from previous partition if any
        output.clear()
    ...
    totalSerializationTime += System.nanoTime() - serializationStartTime;
    shuffleWriteCurator.sendDataBlocks(spillDataItr)
  }
  ...
  private[rss] lazy val shuffleWriteTimeMetadata = ShuffleWriteTimeMetadata(totalSerializationTime,
    shuffleWriteCurator.totalCompressionTime, shuffleWriteCurator.totalUploadTime, totalMemoryFetchWaitTime)

  private[rss] lazy val shuffleWriteMetrics = {
    ShuffleWriteMetadata(recordsReadCnt, recordsWrittenCnt,
      shuffleWriteCurator.numberOfSpills, shuffleWriteCurator.partitionLengths, 0l,
      shuffleWriteCurator.numberOfNetworkWrites)
  }
}
