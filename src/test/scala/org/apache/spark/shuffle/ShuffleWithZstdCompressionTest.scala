 ...
 */
class ShuffleWithZstdCompressionTest {
  ...
  @Test
  def runWithZstd(): Unit = {
    val conf = TestUtil.newSparkConfWithStandAloneRegistryServer(appId, rssTestCluster.getRegistryServerConnection)
    conf.set("spark.shuffle.rss.compress", "zstd")
    runWithConf(conf)
  }

  @Test
  def runWithZstdLevel2(): Unit = {
    val conf = TestUtil.newSparkConfWithStandAloneRegistryServer(appId, rssTestCluster.getRegistryServerConnection)
    conf.set("spark.shuffle.rss.compress", "zstd")
    conf.set("spark.shuffle.rss.compress.zstd.level", "2")
    runWithConf(conf)
  }
