...
public class DataBlockSocketReadClientTest {
    private static final Logger logger = LoggerFactory.getLogger(DataBlockSocketReadClientTest.class);
    ...
    public void connectReturningDataAvailable() {
        ...
        try {
            ...
            try (DataBlockSyncWriteClient writeClient = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appId, appAttempt)) {
                writeClient.connect();
            ...
            try (DataBlockSocketReadClient readClient = new DataBlockSocketReadClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appShufflePartitionId, Arrays.asList(appTaskAttemptId.getTaskAttemptId()), TestConstants.DATA_AVAILABLE_POLL_INTERVAL, TestConstants.DATA_AVAILABLE_TIMEOUT)) {
                ...
                Assert.assertEquals(connectDownloadResponse.getMapTaskCommitStatus().getTaskAttemptIds().stream().findFirst().get(), (Long)taskAttemptId);
    ...
    public void waitDataAvailable() {
        ...
        try {
            ...
            long taskAttemptId = 3;
            int partitionId = 2;
            AppTaskAttemptId appTaskAttemptId1 = new AppTaskAttemptId(appId, appAttempt, shuffleId, mapId, taskAttemptId);

            long taskAttemptId2 = taskAttemptId + 1;
            AppTaskAttemptId appTaskAttemptId2 = new AppTaskAttemptId(appId, appAttempt, shuffleId, mapId, taskAttemptId2);

            // write data without finish upload, server side should not treat data as available

            try (DataBlockSyncWriteClient writeClient = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appId, appAttempt)) {
                writeClient.connect();
                writeClient.startUpload(appTaskAttemptId1.getShuffleMapTaskAttemptId(), numMaps, numPartitions, new ShuffleWriteConfig());
            ...
            AppShufflePartitionId appShufflePartitionId = new AppShufflePartitionId(appId, appAttempt, shuffleId, partitionId);
            DataBlockSocketReadClient readClient = new DataBlockSocketReadClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appShufflePartitionId, Arrays.asList(appTaskAttemptId2.getTaskAttemptId()), TestConstants.DATA_AVAILABLE_POLL_INTERVAL, TestConstants.DATA_AVAILABLE_TIMEOUT);
            ConnectDownloadResponse connectDownloadResponse = readClient.connect();
            ...
            try (DataBlockSyncWriteClient writeClient = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appId, appAttempt)) {
                writeClient.connect();
                writeClient.startUpload(appTaskAttemptId2.getShuffleMapTaskAttemptId(), numMaps, numPartitions, new ShuffleWriteConfig());
            ...
            Assert.assertEquals(getDataAvailabilityResponse.getMapTaskCommitStatus().getTaskAttemptIds().stream().findFirst().get(), (Long)taskAttemptId2);
    ...
    public void readLargeData() {
        ...
        try {
            ...
            try (DataBlockSyncWriteClient writeClient = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", "app1", appAttempt)) {
                ...
                writeClient.writeData(partitionId, appTaskAttemptId.getTaskAttemptId(), Unpooled.wrappedBuffer(bytes));
                writeClient.writeData(partitionId, appTaskAttemptId.getTaskAttemptId(), Unpooled.wrappedBuffer(bytes));
    ...
    public void readData_readBeforeWriteData() {
        ...
        try {
            ...
            AppShufflePartitionId appShufflePartitionId = new AppShufflePartitionId(appId, appAttempt, shuffleId, partitionId);
            try (DataBlockSocketReadClient readClient = new DataBlockSocketReadClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appShufflePartitionId, Arrays.asList(taskAttemptId), TestConstants.DATA_AVAILABLE_POLL_INTERVAL, TestConstants.DATA_AVAILABLE_TIMEOUT)) {
                synchronized (writeDataReadyToRun) {
    ...
    public void readData_ignoreStaleTaskAttemptId() {
        ...
        try {
            ...
            testServer1.getShuffleExecutor().pollAndWaitMapAttemptCommitted(appTaskAttemptId2, TestConstants.DATA_AVAILABLE_TIMEOUT);

            AppShufflePartitionId appShufflePartitionId = new AppShufflePartitionId(appId, appAttempt, shuffleId, partitionId);
            try (DataBlockSocketReadClient readClient = new DataBlockSocketReadClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appShufflePartitionId, Arrays.asList(appTaskAttemptId2.getTaskAttemptId()), TestConstants.DATA_AVAILABLE_POLL_INTERVAL, TestConstants.DATA_AVAILABLE_TIMEOUT)) {
                readClient.connect();
    ...
    public void readData_ignoreStaleTaskAttemptId_oldTaskAttemptWriteAfterNewTaskAttempt() {
        ...
        try {
            ...
            try (DataBlockSyncWriteClient writeClient = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", "app1", appAttempt)) {
                ...
                try (DataBlockSyncWriteClient writeClient2 = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", "app1", appAttempt)) {
                    writeClient2.connect();

                    writeClient2.startUpload(appTaskAttemptId2.getShuffleMapTaskAttemptId(), numMaps, numPartitions, new ShuffleWriteConfig());
    ...
    public void readData_ignoreStaleTaskAttemptId_concurrentWriteWithMultiThreads() {
        ...
        for (int testRound = 0; testRound < testRounds; testRound++) {
            ...
            try {
                ...
                AppShufflePartitionId appShufflePartitionId = new AppShufflePartitionId(appId, appAttempt, shuffleId, partitionId);
                try (DataBlockSocketReadClient readClient = new DataBlockSocketReadClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appShufflePartitionId, Arrays.asList(largestTaskAttemptId), TestConstants.DATA_AVAILABLE_POLL_INTERVAL, TestConstants.DATA_AVAILABLE_TIMEOUT)) {
                    readClient.connect();
                    ...
                    Assert.assertEquals(readData.readableBytes(), lastTaskAttemptWrittenData.readableBytes());
                    Assert.assertEquals(ByteBufUtils.readBytes(readData), ByteBufUtils.readBytes(lastTaskAttemptWrittenData));
                }
    ...
    public void readEmptyFile() throws IOException {
        ...
        try {
            ...
            List<FilePathAndLength> partitionFiles = testServer1.getShuffleExecutor().getPersistedBytes(appShufflePartitionId.getAppShuffleId(), appShufflePartitionId.getPartitionId());
    ...
    public void shuffleDataNotAvailable_secondMapTaskNotFinish() {
        ...
        try {
            ...
            try (DataBlockSyncWriteClient writeClient = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", "app1", appAttempt)) {
                ...
                writeClient.writeData(partitionId, appTaskAttemptId2.getTaskAttemptId(), Unpooled.wrappedBuffer(bytes));
                writeClient.writeData(partitionId, appTaskAttemptId2.getTaskAttemptId(), Unpooled.wrappedBuffer(bytes));
            }
    ...
    public void shuffleDataNotAvailable_secondMapTaskRetryButNotFinish() {
        ...
        try {
            ...
            AppTaskAttemptId appTaskAttemptId = new AppTaskAttemptId(appId, appAttempt, shuffleId, mapId, taskAttemptId);
            AppTaskAttemptId appTaskAttemptId2 = new AppTaskAttemptId(appId, appAttempt, shuffleId, mapId2, taskAttemptId2);
            AppTaskAttemptId appTaskAttemptId3 = new AppTaskAttemptId(appId, appAttempt, shuffleId, mapId2, taskAttemptId3);
            ...
            try (DataBlockSyncWriteClient writeClient = new DataBlockSyncWriteClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", "app1", appAttempt)) {
                ...
                writeClient.startUpload(appTaskAttemptId3.getShuffleMapTaskAttemptId(), numMaps, numPartitions, new ShuffleWriteConfig());

                writeClient.writeData(partitionId, appTaskAttemptId3.getTaskAttemptId(), Unpooled.wrappedBuffer(bytes));
                writeClient.writeData(partitionId, appTaskAttemptId3.getTaskAttemptId(), Unpooled.wrappedBuffer(bytes));
            }

            int dataAvailableWaitTime = 100;
            AppShufflePartitionId appShufflePartitionId = new AppShufflePartitionId(appId, appAttempt, shuffleId, partitionId);
            try (DataBlockSocketReadClient readClient = new DataBlockSocketReadClient("localhost", testServer1.getShufflePort(), TestConstants.NETWORK_TIMEOUT, "user1", appShufflePartitionId, Arrays.asList(appTaskAttemptId.getTaskAttemptId(), appTaskAttemptId3.getTaskAttemptId()), dataAvailableWaitTime/10, dataAvailableWaitTime)) {
                readClient.connect();
                readClient.readDataBlock();
            }
        } finally {
